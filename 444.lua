...
--// GUI (使用 PlayerGui，更稳定)
local gui = Instance.new("ScreenGui")
gui.Name = "AutoBossFightGUI"
...
fightButton.Font = Enum.Font.SourceSansBold
fightButton.TextSize = 16
...
RunService.RenderStepped:Connect(function()
    ...
    if autoFight and currentTarget then
        ...
        if targetHRP and targetHRP.Parent and currentTarget.Parent then
            hrp.Anchored = true
            
            -- 面向 + 保持距离
            hrp.CFrame = CFrame.lookAt(hrp.Position, targetHRP.Position)
            local direction = (hrp.Position - targetHRP.Position).Unit
            hrp.CFrame = CFrame.new(targetHRP.Position + direction * followDistance)
        else
            hrp.Anchored = false
            currentTarget = nil
        end
...
print("Auto Boss Fight 脚本已加载！使用 GUI 操作。")
